...
def get_embeddings(folder_path = 'data/embeddings/intersect'):
    ...
    reference_genes = gene_lists[reference_subfolder]  # get an order of genes that the others will follow
...
def load_annotations(reference_genes, gmt_direct, gmt_prop, slim, obo):
    ...
    annot_direct = {term: [gene for gene in genes if gene in reference_genes] for term, genes in annot_direct.items()}
    annot_prop = {term: [gene for gene in genes if gene in reference_genes] for term, genes in annot_prop.items()}
    ...
    annot_prop_20 = [annot for annot, gene_list in annot_prop.items() if len(gene_list) >= 20]
    ...
    filtered_annot_to_slim = {term: matches for term, matches in annot_to_slim.items() if matches}
    ...
    annot_prop_use = {term: genes for term, genes in annot_prop.items() if term in annot_use}
    annot_slim_use = {term: genes for term, genes in annot_prop.items() if term in slim_set}
    ...
    for annot_term in list(annot_prop_use):
        ...
        if len(other_slims) > 0:
            ...
            for slim_term in other_slims:
                candidate_genes = list(neg_genes.intersection(annot_slim_use[slim_term]))
...
def run_benchmark(reindexed_embeddings, cv_fold1_dict, cv_fold2_dict, cv_fold3_dict, holdout_dict, log_file="gene_level_benchmark_log.txt"):
...
def reformat_results(all_fold_results, all_holdout_results):
    ...
    fold_auc_df = pd.DataFrame(fold_auc_data, index=all_embeddings, columns=all_doid_terms)
    fold_auprc_df = pd.DataFrame(fold_auprc_data, index=all_embeddings, columns=all_doid_terms)
    fold_pr10_df = pd.DataFrame(fold_pr10_data, index=all_embeddings, columns=all_doid_terms)

    holdout_auc_df = pd.DataFrame(holdout_auc_data, index=all_embeddings, columns=all_doid_terms)
    holdout_auprc_df = pd.DataFrame(holdout_auprc_data, index=all_embeddings, columns=all_doid_terms)
    holdout_pr10_df = pd.DataFrame(holdout_pr10_data, index=all_embeddings, columns=all_doid_terms)

    holdout_time_df = pd.DataFrame(holdout_time_data, index=all_embeddings, columns=all_doid_terms)
    ...
    return df_summary_with_meta, fold_auc_df, fold_auprc_df, fold_pr10_df, holdout_auc_df, holdout_auprc_df, holdout_pr10_df, holdout_time_df
...
def plot_slim_clustermap(
    ...
    max_slims = max(len(slims) for slims in filtered_annot_to_slim.values()) if filtered_annot_to_slim else 0
    all_slims = {
        slim_term
